...
class DDIMScheduler(DiscreteScheduler):
    ...
    def step(
        ...
        **kwargs
        ...
        # "predicted x_0" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf
        if self.parameterization == "eps":
            pred_original_sample = (x - beta_prod_t ** (0.5) * e_t) / a_t ** (0.5)
            pred_epsilon = e_t
        elif self.parameterization == "x0":
            pred_original_sample = e_t
            pred_epsilon = (x - a_t ** (0.5) * pred_original_sample) / beta_prod_t ** (0.5)
        elif self.parameterization== "v":
            pred_original_sample = (a_t**0.5) * x - (beta_prod_t**0.5) * e_t
            pred_epsilon = (a_t**0.5) * e_t + (beta_prod_t**0.5) * x
        else:
            raise ValueError(
