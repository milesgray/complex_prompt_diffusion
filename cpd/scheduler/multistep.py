...
class DPMSolverMultistepScheduler:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.beta_end = kwargs.get("beta_end", 0.01)
        self.solver_order = kwargs.get("solver_order", 2)
        self.algorithm_type = kwargs.get("algorithm_type", "dpmsolver++")
        self.solver_type = kwargs.get("solver_type", "midpoint")
        self.lower_order_final = kwargs.get("lower_order_final", True)
        self.verbose = kwargs.get("verbose", None)

        if self.trained_betas is not None:
            self.betas = torch.from_numpy(self.trained_betas)
        elif self.beta_schedule == "linear":
            self.betas = torch.linspace(self.beta_start, self.beta_end, num_train_timesteps, dtype=torch.float32)
        elif self.beta_schedule == "scaled_linear":
            # this schedule is very specific to the latent diffusion model.
            self.betas = (
                torch.linspace(self.beta_start**0.5, self.beta_end**0.5, num_train_timesteps, dtype=torch.float32) ** 2
            )
        ...
        if self.algorithm_type not in ["dpmsolver", "dpmsolver++"]:
            raise NotImplementedError(f"{self.algorithm_type} does is not implemented for {self.__class__}")
        if self.solver_type not in ["midpoint", "heun"]:
            raise NotImplementedError(f"{self.solver_type} does is not implemented for {self.__class__}")
    ...
    def set_timesteps(self, 
                    ...
                    **kwargs):
        """
        Set the timesteps used for the diffusion chain. Supporting function to be run before inference.
        ...
        **kwargs
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if self.thresholding:
                if self.clip_sample:
                    sample_thresholding = create_extension(self.clip_sample_alg)
                    x0_pred = sample_thresholding(x0_pred, timestep, threshold=self.clip_sample_thresh)
                else:
    ...
    def step(
        ...
        **kwargs
        ...
        self.algorithm_type = kwargs.get("multistep_alg_type", "dpmsolver++")
        self.solver_type = kwargs.get("multistep_solver_type", "heun")
        self.thresholding = kwargs.get("multistep_dynamic_thresh", False)
        self.clip_sample = kwargs.get("clip_sample", False)
        self.clip_sample_alg = kwargs.get("clip_sample_alg", "dynamic_thresholding")
        self.clip_sample_thresh = kwargs.get("clip_sample_thresh", 90)
        self.predict_epsilon = kwargs.get("predic_epsilon", True)
        ...
        self.timesteps = safe_to(self.timesteps)
